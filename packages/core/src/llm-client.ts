...
/**
 * Ask a provider to distill the prompt into a KbEntryDraft, forcing the model
 * to return JSON that matches our schema. Dispatches on the provider's API
 * flavor; the two request/response shapes are the only provider-specific code.
 */
export const requestKbDraft = async (req: StructuredRequest): Promise<KbEntryDraft> => {
  return req.provider.flavor === "anthropic"
    ? requestViaAnthropic(req)
    : requestViaOpenAI(req);
};
...
const requestViaOpenAI = async (req: StructuredRequest): Promise<KbEntryDraft> => {
  ...
  if (!argsJson) throw new Error("model did not return a structured KB entry");
  try {
